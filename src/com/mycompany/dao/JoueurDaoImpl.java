...
public class JoueurDaoImpl extends TennisDaoBaseClass implements JoueurDao
{
    ...
    @Override
    public List<Joueur> rechercher(String searchQuery, Filtre f) throws DaoException
    {
        Connection connection = null;
        PreparedStatement statement = null;
        String sqlStr = "SELECT * FROM JOUEUR WHERE (JOUEUR.NOM LIKE ? OR JOUEUR.PRENOM LIKE ?) AND SEXE LIKE ?;";
        List<Joueur> joueurs = new ArrayList<>();
        try {
            connection = daoFactory.getConnection();
            statement = connection.prepareStatement(sqlStr);
            ...
            statement.setString(3, f.getSexe() );
            ResultSet rs = statement.executeQuery();
            while (rs.next() ) {
                joueurs.add(new Joueur(rs) );
            }
